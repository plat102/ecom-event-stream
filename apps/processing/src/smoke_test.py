"""
Spark connectivity smoke test: confirm Spark can reach Kafka and PostgreSQL before writing the real job.
...
def main():
    ...
    log.info("JDBC read: dim_date")
    dim_date = spark.read.jdbc(
        url=jdbc_url(settings), table="dim_date", properties=jdbc_properties(settings)
    )
    dim_date.show(5)
